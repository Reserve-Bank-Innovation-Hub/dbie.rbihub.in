// Processor: area-under-cultivation-foodgrains — RBI Handbook annual series.
//
// Source xlsx carries one sheet with annual area under cultivation figures for foodgrains,
// newest-first from 2025-26 back to 1950-51. Layout:
//   row 1  blank
//   row 2  title: "Area Under Cultivation - Foodgrains"
//   row 3  blank
//   row 4  units: "(Lakhs Hectares)"
//   row 5  blank
//   row 6  header row 1: Year | Cereals (Rice, Wheat, Coarse Cereals, Total) | Pulses
...
//   col[5] = total_cereals  (labelled "Total (2+3+4)")
//   col[6] = pulses
//
// Emits (newest-first):
//   out/area-under-cultivation-foodgrains.json
...
const XLSX_SRC = path.join(
  __dirname,
  '../publications/handbook-statistics-on-indian-economy/annual-series/output-and-prices/Area Under Cultivation - Foodgrains.xlsx',
);
...
function parse(buf) {
  ...
  for (const sn of wb.SheetNames) {
    ...
    for (let i = 0; i < Math.min(rows.length, 12); i++) {
      ...
      if (/Area Under Cultivation.*Foodgrains/i.test(c1)) {
        reportTitle = c1;
      }
      if (/Lakhs/i.test(c1)) {
    ...
    return {
      reportTitle : reportTitle || 'Area Under Cultivation - Foodgrains',
      units,
      data,
    };
  }
...
function selfCheck(out) {
  ...
  const known = [
    { year: '2025-26', field: 'rice',          value: 528.32   },
    { year: '2025-26', field: 'coarse_cereals',value: 272.47   },
    { year: '2024-25', field: 'wheat',         value: 328.04  },
    { year: '2024-25', field: 'total_cereals', value: 1098.26 },
    { year: '1950-51', field: 'rice',          value: 308.1   },
    { year: '1950-51', field: 'pulses',        value: 190.9   },
  ];
...
function main() {
  ...
  if (errors.length > 0) {
    console.error('area-under-cultivation-foodgrains self-check FAILED:');
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, 'area-under-cultivation-foodgrains.json'),
    JSON.stringify(out),
  );
