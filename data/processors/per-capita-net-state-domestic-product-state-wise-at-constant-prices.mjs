// Processor: per-capita-net-state-domestic-product-state-wise-at-constant-prices (annual).
//
// Source xlsx — "Per Capita Net State Domestic Product - State-wise (At Constant Prices).xlsx"
...
//
// Values are per-capita rupees — decimal strings (e.g. "131082.8993").
// Unit is "Rupees" (not "Rupees Crores").
// Header col[1] label is "STATE / UNION TERRITORY" (vs "Year" for the totals tables).
// This table includes 33 states/UTs (adds Puducherry compared with the NSDP totals).
//
// Emits:
//   out/per-capita-net-state-domestic-product-state-wise-at-constant-prices.json
//   {
//     reportTitle : string,
//     baseYear    : string,
//     unit        : string,
...
const XLSX_SRC = path.join(
    ...
    'Per Capita Net State Domestic Product - State-wise (At Constant Prices).xlsx',
);
const OUT_DIR = path.join(__dirname, 'out');

// Decimal strings like "131082.8993"; strip commas defensively.
...
function parse(buf) {
    ...
    // Locate the column-header row: the per-capita files use "STATE / UNION TERRITORY"
    // as the first-column label instead of "Year".
    let headerRow = -1;
    let states = [];
    ...
    for (let i = 0; i < Math.min(rows.length, 12); i++) {
        ...
        if (/^Year$/i.test(c1) || /^STATE\s*\/\s*UNION/i.test(c1) || /^State\s*\/\s*Union/i.test(c1)) {
            headerRow = i;
            // States live in cols 2 onwards; exclude numeric serial labels.
    ...
    return {
        reportTitle : 'Per capita Net State Domestic Product — state-wise (at constant prices)',
        baseYear    : '2011-12',
        unit        : 'Rupees',
        years,
        states,
...
function selfCheck(out) {
    ...
    // Assertion 1: 2023-24, Andhra Pradesh ≈ 131082.8993.
    {
        const idx = yearIdx(out, '2023-24');
        const val = data['Andhra Pradesh'][idx];
        if (val == null || !approxEq(val, 131082.8993)) {
            fail(`2023-24 Andhra Pradesh: expected ≈131082.8993, got ${val}`);
        }
    }

    // Assertion 2: 2023-24, Gujarat ≈ 195617.1475.
    {
        const idx = yearIdx(out, '2023-24');
        const val = data['Gujarat'][idx];
        if (val == null || !approxEq(val, 195617.1475)) {
            fail(`2023-24 Gujarat: expected ≈195617.1475, got ${val}`);
        }
    }
...
function main() {
    ...
    const outPath = path.join(OUT_DIR, 'per-capita-net-state-domestic-product-state-wise-at-constant-prices.json');
